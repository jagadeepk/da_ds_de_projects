...
def generate_sql_query_route():
  ...
  filters=["this month"]
  metrics_output=["Impressions","Clicks","Cost","Conversions","Conversion value"]
  metrics_mapping ={'Impressions': 'metrics_impressions', 'Clicks': 'metrics_clicks'
                    , 'Cost': 'metrics_cost_micros','Conversions': 'metrics_conversions'
                    ,'Conversion value':'metrics_conversions_value'}
  metrics=list(map(metrics_mapping.get,metrics_output))
  sql_query = generate_sql_query(categorizations, filters, metrics)
  print("in app route",sql_query)
  return sql_query
